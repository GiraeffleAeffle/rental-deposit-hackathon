...
The existing joint-signature application agreement has proved setup, funding and supply and remains active. A different Privy agreement proved staged setup, funding, Kamino supply/redemption and zero-claim acceptance. Its finalized on-chain state is `settling` with 10 test USDC cash, zero receipts and zero approved landlord allocation. Final payout remains unproved. The independent operator rehearsal does not grant this server authority over its tenancy or user wallets.
