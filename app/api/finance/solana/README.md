...
The existing joint-signature application agreement has proved setup, funding and supply. A different Privy agreement proved staged setup only; its finalized on-chain state holds zero security. Neither app agreement has proved a Privy-connected exit. The independent operator rehearsal does not grant this server authority over its tenancy or user wallets.
