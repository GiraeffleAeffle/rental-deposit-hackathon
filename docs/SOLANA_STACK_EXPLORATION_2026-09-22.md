...
| Devnet            | KLend executable program and Circle test USDC exist.                                                                | No compatible initialized USDC reserve or issuer xStock market was verified. Mainnet accounts and routes cannot simply be reused here. |
...
1. **Pin the deployment manifest.** Decode the selected reserve and all required authorities, supply accounts, receipt mint, oracles, caps, queues and program version. Reject wrong owners/cluster/mints in a table-driven test. Determine whether an existing devnet reserve supports the exact test USDC mint; otherwise plan a controlled local fixture and label it.
