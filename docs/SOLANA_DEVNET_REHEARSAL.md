...
This records the 23 September 2026 **test-token** proofs. An operator-key tenancy completed a no-claim custody cycle against a real Kamino devnet reserve. A separate Privy-connected application tenancy initialized, funded and supplied 10 test USDC. Neither run establishes earned yield, a tokenized-stock purchase, a Privy-connected settlement, or production readiness. See the [deployment](evidence/SOLANA_DEVNET_DEPLOYMENT_2026-09-23.json) and [operator transaction evidence](evidence/SOLANA_DEVNET_ESCROW_REHEARSAL_2026-09-23.json).
...
These application receipts and wallet addresses remain in ignored local records, not in the public evidence files, because publishing them would link the user's Privy accounts. The application position has not earned a measured surplus. Redemption, earnings release, claims, settlement and a personal investment order have not been completed with these Privy accounts. The finished operator-key custody cycle above is separate evidence, not a substitute for those remaining application actions.
...
For a **new application** agreement, create separate Privy tenant, landlord and arbitrator accounts and accept the terms with the actual original wallet identities. Precreate the tenant and landlord associated test-USDC payout accounts, derive the new tenancy PDA from the agreement ID and tenant wallet, and configure `SOLANA_DEPLOYMENT_MANIFEST` with the pinned deployment plus that `agreementId` and `tenancyAddress`. The **Initialize the Solana tenancy** panel prepares one message: tenant and landlord must each review and sign it before the blockhash expires; the server's separate sponsor pays rent and fees. Verify its receipt and initialized account before funding. The connected milestone above demonstrates this through supply for one application tenancy. The issuer xStock route is not available on this devnet proof; personal buy/sell, eligibility and cash exit remain separate gates.
