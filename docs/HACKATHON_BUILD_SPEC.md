...
The next Solana onboarding slice is the [staged tenancy journey](STAGED_SOLANA_ONBOARDING.md). It replaces synchronized setup signatures on a **separate** devnet program while preserving the funded tenancy on its pinned program. Landlord-only creation of an empty escrow has finalized; completion still requires later tenant funding and exit with independently verified receipts.
