...
The next Solana onboarding slice is the [staged tenancy journey](STAGED_SOLANA_ONBOARDING.md). It replaces synchronized setup signatures for a **new** test deployment while preserving the funded tenancy on its pinned program. Completion requires asynchronous landlord setup and tenant funding with independently verified receipts; the document is a target, not deployed behavior.
