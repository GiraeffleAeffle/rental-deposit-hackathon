...
The next Solana onboarding slice is the [staged tenancy journey](STAGED_SOLANA_ONBOARDING.md). It replaces synchronized setup signatures on a **separate** devnet program while preserving the funded tenancy on its pinned program. Landlord-only creation, later tenant funding, Kamino supply/redemption and zero-claim acceptance have finalized with independently verified account state. Final payout remains to be proved.
