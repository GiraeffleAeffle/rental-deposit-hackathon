...
| Access                  | Privy passkey/backup hooks, explicit original-wallet creation, verified JWT/user/sole-owner wallet checks, actual EIP-191/Ed25519 recovery challenges  | Recovery from another physical device and cancelled-signature rehearsal                                        |
...
The user created EVM and Solana embedded wallets, and the browser displayed both network addresses. A browser-driven `GET /api/identity` passed the server's Privy token, current-user and sole-owner wallet checks and returned the original-wallet enrollment step. `POST /api/identity/baseline` then recorded both original wallets. The user completed backup-email sign-in in Chrome and approved both recovery messages in that new browser/session. A fresh identity check in the enrollment browser returned "Same-wallet access verified in another browser" from the persisted server proof. No wallet address or provider secret is published in this repository. Recovery from another physical device remains unproved. A clean checkout or hosted deployment still needs its own secure configuration. The [wallet setup guide](WALLET_SETUP.md) records the remaining steps.
