...
The original joint-signature devnet program is `BiwaGavQUsSsg48UPpRAGWoXSiUnzgvdDs7rd8WizvPD`; its funded tenancy remains there. The staged source is separately deployed at `B1hjmapwssey8AbpjAtw5qF87DvvtuSisGov4kHec7Yc`. The [staged deployment record](../../docs/evidence/SOLANA_STAGED_DEVNET_DEPLOYMENT_2026-09-24.json) pins its finalized executable hash, authority and reserve. `Anchor.toml` selects localnet. On the original operator machine, the matching test program keypairs and reviewed `.so` are preserved in the ignored `.testnet-secrets/solana/` directory; none are committed or supplied to other checkouts. The keypair files are mode 0600. The client requires an explicit deployment manifest, genesis hash and reviewed program hash before it can plan test-network writes. One Privy-backed tenancy on the staged program has finalized setup, funding, supply, redemption and zero-claim acceptance; final payout is pending.
...
| Initialize staged (separate test program) | The landlord signs creation of an empty escrow after off-chain agreement acceptance. Tenant is fixed as a non-signer and later signs `fund`; the separate payer covers rent. A connected tenancy has proved this asynchronous setup and tenant funding, then Kamino supply/redemption and zero-claim acceptance. |
...
The [SBF CI job](../../.github/workflows/solana-sbf.yml) repeats this build and local SVM proof on an isolated runner using the SHA-256-pinned Agave v4.2.2 release. The separately finalized staged deployment was checked byte for byte against that artifact and its upgrade authority was verified. The original program and its funded tenancy were not upgraded.
