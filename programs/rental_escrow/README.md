...
| Initialize staged (separate test program) | The landlord signs creation of an empty escrow after off-chain agreement acceptance. Tenant is fixed as a non-signer and must later sign `fund`; the separate payer covers rent. One connected landlord setup finalized with zero security; the tenant-funded journey remains to be proved. |
